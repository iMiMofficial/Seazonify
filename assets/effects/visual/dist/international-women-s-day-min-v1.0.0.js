...
css: `.szfy-iwd-container{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9999;overflow:hidden;}.szfy-iwd-edge-flower{position:fixed;pointer-events:none;will-change:transform,opacity;filter:drop-shadow(0 6px 15px rgba(0,0,0,0.25)) drop-shadow(0 2px 5px rgba(255,64,129,0.4));z-index:9990;opacity:0;transition:opacity 2s ease,transform 4s ease-out;}.szfy-iwd-edge-flower svg{display:block;width:100%;height:100%;}.szfy-iwd-trail-particle{position:absolute;pointer-events:none;animation:szfy-iwd-trail-drift var(--tdur) ease-out forwards;will-change:transform,opacity;z-index:90;}.szfy-iwd-trail-particle svg{width:100%;height:100%;filter:drop-shadow(0 2px 8px rgba(255,64,129,0.4));}@keyframes szfy-iwd-trail-drift{0%{transform:translate(0,0) scale(var(--s1)) rotate(0deg);opacity:0;}15%{opacity:var(--topa);}100%{transform:translate(var(--tx),var(--ty)) scale(var(--s2)) rotate(var(--tspin));opacity:0;}}.szfy-iwd-butterfly{position:absolute;top:0;left:0;pointer-events:none;will-change:transform;filter:drop-shadow(0 4px 10px rgba(0,0,0,0.2)) drop-shadow(0 1px 3px rgba(255,64,129,0.3));z-index:110;}.szfy-iwd-butterfly svg{display:block;width:100%;height:100%;}.szfy-iwd-bg-particle{position:fixed;pointer-events:none;z-index:9991;filter:drop-shadow(0 3px 8px rgba(0,0,0,0.2));}@keyframes szfyHeroPulse{0%{transform:scale(0.9);opacity:0.3;filter:brightness(1) drop-shadow(0 0 4px rgba(255,255,255,0.4));}100%{transform:scale(1.15);opacity:0.9;filter:brightness(1.5) drop-shadow(0 0 10px rgba(255,255,255,0.9));}}@media (prefers-reduced-motion:reduce){.szfy-iwd-hero,.szfy-iwd-trail-particle,.szfy-iwd-butterfly{display:none !important;}}`,
...
var BUTTERFLY_POOL = MAX_ON_SCREEN;
...
if (randVal < 0.6) this.hue = rand(320, 360);
...
this.state = 'sitting';
activeSittingCount++;
this.sitTimer = rand(100, 900);
this.isFlipping = Math.random() < 0.4;
} else {
this.pickNewTarget();
}
} else {
...
bgFlowers.push({
x: centerX + rand(-cellW * 0.4, cellW * 0.4),
