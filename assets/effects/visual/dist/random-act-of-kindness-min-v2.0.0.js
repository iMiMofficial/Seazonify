...
css: `.szfy-kindness-container{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:9999;overflow:hidden;}.szfy-kindness-heart{position:absolute;opacity:0;pointer-events:none;will-change:transform,opacity;animation:szfy-kindness-rise var(--szfy-k-dur) ease-in-out forwards,szfy-kindness-drift var(--szfy-k-drift-dur) ease-in-out infinite alternate;}.szfy-kindness-heart svg{display:block;width:100%;height:100%;filter:drop-shadow(0 0 var(--szfy-k-glow-size) var(--szfy-k-glow));}@keyframes szfy-kindness-rise{0%{transform:translateY(0) scale(var(--szfy-k-scale-start));opacity:0;}8%{opacity:var(--szfy-k-opacity);}85%{opacity:var(--szfy-k-opacity);}100%{transform:translateY(var(--szfy-k-travel)) scale(var(--szfy-k-scale-end));opacity:0;}}@keyframes szfy-kindness-drift{0%{margin-left:calc(var(--szfy-k-drift) * -1);}100%{margin-left:var(--szfy-k-drift);}}.szfy-kindness-orb{position:absolute;border-radius:50%;pointer-events:none;will-change:transform,opacity;background:radial-gradient(circle at 35% 35%,var(--szfy-k-orb-inner),var(--szfy-k-orb-outer) 70%,transparent 100%);filter:blur(1px);animation:szfy-kindness-orb-float var(--szfy-k-orb-dur) ease-in-out forwards,szfy-kindness-orb-pulse var(--szfy-k-orb-pulse-dur) ease-in-out infinite alternate;opacity:0;}@keyframes szfy-kindness-orb-float{0%{transform:translateY(0) scale(0.5);opacity:0;}12%{opacity:var(--szfy-k-orb-opacity);}80%{opacity:var(--szfy-k-orb-opacity);}100%{transform:translateY(var(--szfy-k-orb-travel)) scale(0.3);opacity:0;}}@keyframes szfy-kindness-orb-pulse{0%{transform:scale(1);}100%{transform:scale(1.15);}}.szfy-kindness-sparkle{position:absolute;width:var(--szfy-k-sp-size);height:var(--szfy-k-sp-size);pointer-events:none;will-change:transform,opacity;opacity:0;}.szfy-kindness-sparkle::before,.szfy-kindness-sparkle::after{content:'';position:absolute;background:var(--szfy-k-sp-color);border-radius:50%;}.szfy-kindness-sparkle::before{top:0;left:50%;width:2px;height:100%;transform:translateX(-50%);}.szfy-kindness-sparkle::after{top:50%;left:0;width:100%;height:2px;transform:translateY(-50%);}.szfy-kindness-sparkle{animation:szfy-kindness-twinkle var(--szfy-k-sp-dur) ease-in-out forwards;}@keyframes szfy-kindness-twinkle{0%{transform:scale(0) rotate(0deg);opacity:0;}20%{transform:scale(1) rotate(20deg);opacity:var(--szfy-k-sp-opacity);}50%{transform:scale(1.2) rotate(45deg);opacity:var(--szfy-k-sp-opacity);}80%{transform:scale(0.8) rotate(65deg);opacity:calc(var(--szfy-k-sp-opacity) * 0.5);}100%{transform:scale(0) rotate(90deg);opacity:0;}}.szfy-kindness-wish{position:fixed;top:50%;left:0;width:100%;transform:translateY(-50%);pointer-events:none;z-index:10000;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;opacity:0;animation:szfy-kindness-wish-life 8s ease-in-out forwards;}.szfy-kindness-wish-main{font-family:'Georgia','Times New Roman',serif;font-size:clamp(22px,4vw,42px);font-weight:700;font-style:italic;color:transparent;background:linear-gradient(135deg,#FF6B81,#FF8FA3,#FFACC7,#FF7EB3);-webkit-background-clip:text;background-clip:text;text-align:center;line-height:1.3;padding:0 20px;filter:drop-shadow(0 0 18px rgba(255,107,129,0.35)) drop-shadow(0 2px 6px rgba(0,0,0,0.08));letter-spacing:0.5px;transform:translateX(60px);opacity:0;animation:szfy-kindness-text-slide 8s ease-in-out forwards;}.szfy-kindness-wish-sub{font-family:'Georgia','Times New Roman',serif;font-size:clamp(11px,1.6vw,16px);font-weight:400;color:rgba(255,143,163,0.8);text-align:center;letter-spacing:2px;text-transform:uppercase;padding:0 20px;filter:drop-shadow(0 0 8px rgba(255,143,163,0.25));transform:translateX(80px);opacity:0;animation:szfy-kindness-text-slide 8s ease-in-out 0.3s forwards;}.szfy-kindness-wish-heart-icon{font-size:clamp(16px,2.5vw,26px);opacity:0;animation:szfy-kindness-icon-pop 8s ease-in-out 0.15s forwards;filter:drop-shadow(0 0 10px rgba(255,107,129,0.4));}@keyframes szfy-kindness-wish-life{0%{opacity:0;}8%{opacity:1;}85%{opacity:1;}100%{opacity:0;}}@keyframes szfy-kindness-text-slide{0%{transform:translateX(60px);opacity:0;}10%{transform:translateX(0);opacity:1;}82%{transform:translateX(0);opacity:1;}100%{transform:translateX(-60px);opacity:0;}}@keyframes szfy-kindness-icon-pop{0%{transform:scale(0.3);opacity:0;}12%{transform:scale(1.15);opacity:1;}18%{transform:scale(1);}80%{transform:scale(1);opacity:1;}100%{transform:scale(0.3);opacity:0;}}`,
