// Valentine's Day Effect v1.0.0 for Seazonify Controller
// Romantic atmosphere — rose petals, glowing hearts, warm bokeh, love letters
// Performance-optimized with 3-tier device detection
// Compatible with SeazonifyController.injectVisualEffect()
// Usage: SeazonifyController.injectVisualEffect(valentinesDayEffect);
...
const valentinesDayEffect = {
  ...
  css: `
    ...
    .szfy-val-vignette {
      ...
      background: radial-gradient(ellipse at center,
        transparent 45%,
        rgba(160,20,40,0.04) 70%,
        rgba(100,10,20,0.08) 100%);
      animation: szfy-val-vig-breathe 6s ease-in-out infinite alternate;
    }
    @keyframes szfy-val-vig-breathe {
      0%   { opacity: 0.5; }
      100% { opacity: 1; }
    }
    ...
    .szfy-val-petal {
      ...
      opacity: 0;
      animation: szfy-val-petal-fall var(--vp-dur) ease-in-out forwards;
    }
    .szfy-val-petal-inner {
      width: 100%; height: 100%;
      border-radius: 62% 2% 48% 50% / 55% 2% 48% 52%;
      background: var(--vp-bg);
      position: relative;
      animation: szfy-val-petal-spin var(--vp-spin-dur) linear infinite;
    }
    .szfy-val-petal-inner::after {
      content: '';
      position: absolute;
      top: 35%; left: 20%;
      width: 45%; height: 1px;
      background: linear-gradient(90deg, transparent, var(--vp-vein), transparent);
      transform: rotate(-12deg);
      opacity: 0.4;
    }
    @keyframes szfy-val-petal-fall {
      0%   { transform: translateY(0) translateX(0); opacity: 0; }
      5%   { opacity: var(--vp-opa); }
      30%  { transform: translateY(30vh) translateX(var(--vp-drift1)); }
      60%  { transform: translateY(60vh) translateX(var(--vp-drift2)); }
      90%  { opacity: var(--vp-opa); }
      100% { transform: translateY(calc(100vh + 60px)) translateX(var(--vp-drift3)); opacity: 0; }
    }
    @keyframes szfy-val-petal-spin {
    ...
    .szfy-val-heart {
      ...
      opacity: 0;
      animation:
        szfy-val-heart-rise var(--vh-dur) ease-in-out forwards,
        szfy-val-heart-sway var(--vh-sway-dur) ease-in-out infinite alternate;
    }
    .szfy-val-heart svg {
      display: block;
      width: 100%; height: 100%;
      filter: drop-shadow(0 0 var(--vh-glow-r) var(--vh-glow));
    }
    .szfy-val-heart-beat {
      animation: szfy-val-heartbeat 1.1s ease-in-out infinite;
    }
    @keyframes szfy-val-heart-rise {
      0%   { transform: translateY(0) scale(0.5); opacity: 0; }
      7%   { opacity: var(--vh-opa); }
      50%  { transform: translateY(calc(var(--vh-travel) * 0.5)) scale(1.05); }
      88%  { opacity: var(--vh-opa); }
      100% { transform: translateY(var(--vh-travel)) scale(0.4); opacity: 0; }
    }
    @keyframes szfy-val-heart-sway {
      0%   { margin-left: calc(var(--vh-sway) * -1); }
      100% { margin-left: var(--vh-sway); }
    }
    @keyframes szfy-val-heartbeat {
      0%   { transform: scale(1); }
    ...
    /* ── Bokeh — static blur only, no animated filter ── */
    .szfy-val-bokeh {
      position: absolute;
      pointer-events: none;
      border-radius: 50%;
      ...
      filter: blur(var(--vbk-blur));
      animation: szfy-val-bokeh-appear var(--vbk-dur) ease-in-out forwards;
    }
    @keyframes szfy-val-bokeh-appear {
      0%   { transform: scale(0.3); opacity: 0; }
      12%  { opacity: var(--vbk-opa); }
      50%  { transform: scale(1); }
      80%  { opacity: var(--vbk-opa); }
      100% { transform: scale(0.4); opacity: 0; }
    }

    /* ── Love Letter ── */
    .szfy-val-letter-wrap {
      position: fixed;
      pointer-events: none;
      z-index: 10000;
      opacity: 0;
    }
    .szfy-val-letter-wrap.szfy-active {
      animation: szfy-val-letter-anim 10s ease-in-out forwards;
    }
    .szfy-val-envelope {
      position: relative;
      width: clamp(220px, 44vw, 380px);
      padding: 22px 24px 18px;
      background: linear-gradient(165deg, rgba(255,248,248,0.93), rgba(255,238,240,0.89));
      border-radius: 6px;
      border-top: 3px solid #D42145;
      box-shadow: 0 4px 20px rgba(160,20,40,0.08);
    }
    .szfy-val-seal {
      position: absolute;
      top: -12px; right: 18px;
      width: 24px; height: 24px;
      background: radial-gradient(circle, #E63946 35%, #B81D3A);
      border-radius: 50%;
      display: flex; align-items: center; justify-content: center;
      font-size: 10px;
      box-shadow: 0 2px 6px rgba(200,30,50,0.3);
    }
    .szfy-val-letter-quote {
    ...
    .szfy-val-letter-line {
      width: 36px; height: 1px;
      background: linear-gradient(90deg, transparent, rgba(180,25,50,0.25), transparent);
      margin: 7px auto 9px;
    }
    .szfy-val-letter-attr {
      font-family: 'Georgia', serif;
      font-size: clamp(9px, 1.2vw, 12px);
      color: rgba(122,21,40,0.5);
      text-align: center;
      letter-spacing: 1.5px;
      text-transform: uppercase;
    }
    /* Letter positions */
    .szfy-val-letter-wrap.szfy-pos-bl { bottom: 5vh; left: 3vw; }
    .szfy-val-letter-wrap.szfy-pos-br { bottom: 5vh; right: 3vw; }
    .szfy-val-letter-wrap.szfy-pos-tl { top: 5vh; left: 3vw; }
    .szfy-val-letter-wrap.szfy-pos-tr { top: 5vh; right: 3vw; }
    .szfy-val-letter-wrap.szfy-pos-bc { bottom: 5vh; left: 50%; transform: translateX(-50%); }
    .szfy-val-letter-wrap.szfy-pos-tc { top: 5vh; left: 50%; transform: translateX(-50%); }

    @keyframes szfy-val-letter-anim {
      0%   { opacity: 0; transform: var(--vl-enter); }
      10%  { opacity: 1; transform: var(--vl-rest); }
      85%  { opacity: 1; transform: var(--vl-rest); }
      100% { opacity: 0; transform: var(--vl-exit); }
    }
    .szfy-val-letter-wrap.szfy-pos-bc.szfy-active,
    .szfy-val-letter-wrap.szfy-pos-tc.szfy-active {
      animation: szfy-val-letter-anim-c 10s ease-in-out forwards;
    }
    @keyframes szfy-val-letter-anim-c {
  ...
  js: `
    (function() {
      ...
      /* ── Device tier: small / medium / large ── */
      function getDeviceTier() {
        ...
        /* Small: phones, small tablets, low-res */
        if (w < 480 || (isTouch && pixels < 400000) || (isTouch && dpr < 2 && w < 768))
          return 'small';
        /* Medium: tablets, mid phones, smaller desktops */
        if (w < 1024 || (isTouch && w < 1200))
          return 'medium';
        /* Large: desktops, large tablets in landscape */
        return 'large';
      }
